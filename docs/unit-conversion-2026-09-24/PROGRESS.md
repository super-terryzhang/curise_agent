...
系统不会从 `KG2.2`、`CA2.27` 的数字后缀自行猜测业务含义。当前只创建了 5 条供管理员审核的草稿规则，`verified=0`；因此生产自动匹配行为仍保守地把 53 个已匹配但单位关系未确认的行标为 `UNIT_CONVERSION_REQUIRED`。
...
- 请用户在正式页面验收 PO165047CCI 的问题展示、当前行处理和规则管理交互。
- 五条草稿需要管理员依据真实业务证据逐条决定验证、修改或停用；系统不会代替该业务决策。
