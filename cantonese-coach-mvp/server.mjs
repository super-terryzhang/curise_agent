...
http.createServer(async function(req,res){
 ...
 try{
  ...
  if(u.pathname==='/api/tts'){
   ...
   console.log('[tts] upstream start',{textLength:String(d.text||'').length,jyutpingLength:String(d.jyutping||'').length});
   var started=Date.now();
   var r=await fetch(CAI+'/tts',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({api_key:k,text:String(d.text||''),jyutping:String(d.jyutping||''),model_id:'v6',frame_rate:'24000',speed:.92,pitch:0,language:'cantonese',output_extension:'wav',should_return_timestamp:false}),signal:AbortSignal.timeout(20000)});
   console.log('[tts] upstream response',{status:r.status,contentType:r.headers.get('content-type'),ms:Date.now()-started});
   if(!r.ok){var x=await upstream(r);console.log('[tts] upstream error',JSON.stringify(x).slice(0,500));return js(res,r.status,{error:err(x,'TTS 失敗')})}
   var buf=Buffer.from(await r.arrayBuffer());console.log('[tts] bytes',buf.length);return send(res,200,buf,r.headers.get('content-type')||'audio/wav')
  }
