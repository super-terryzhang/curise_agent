...
# ---- Build-only VITS: generate the two fixed reference sentences, then delete model ----
...
refs=[REF/"line0.wav",REF/"line1.wav"]
need_refs=not all(p.exists() and p.stat().st_size>3000 for p in refs)
if need_refs:
    ...
    for i,text in enumerate(lines):
        print(f"[reference] generating line{i+1}",flush=True)
        audio=tts.generate(text=t2s.convert(text),sid=0,speed=.88)
        samples=np.asarray(audio.samples,dtype=np.float32)
        if samples.size<100:raise RuntimeError(f"Reference line{i+1} generated empty audio")
        pcm=(np.clip(samples,-1,1)*32767).astype("<i2")
        with wave.open(str(refs[i]),"wb") as w:
            w.setnchannels(1);w.setsampwidth(2);w.setframerate(int(audio.sample_rate));w.writeframes(pcm.tobytes())
        print(f"[reference] line{i+1} ready bytes={refs[i].stat().st_size}",flush=True)
    del tts
else:
    print("[reference] reuse pre-generated sentence WAVs",flush=True)
